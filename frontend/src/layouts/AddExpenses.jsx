...
function AddExpenses() {
    ...
    useEffect(() => {
        ...
        const eventSource = new EventSource(`${process.env.REACT_APP_API_URL}/expenses/stream`);

        eventSource.onmessage = (event) => {
            const newExpense = JSON.parse(event.data);
            setExpenses((prev) => [...prev, newExpense]); // add new expense instantly
            toast.info(`New expense added: ${newExpense.name} - ${newExpense.amount}`, { position: "top-right" });
        };
    ...
    return (
        <div>
            ...
            >
                        ...
                        <table className="min-w-full bg-white">
                            ...
                            <tbody>
                                {expenses.map((expense, index) => (
                                    <tr key={expense.id || index} className="text-center border hover:bg-gray-100">
                ...
                {/* Expense Modal */}
                <ExpensesForm isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
            </div>
        </div>
    );
}
